...
static atomic_uint g_max_finished_p = 0; // Tracks the highest successfully verified exponent
...
/* External linkage for Gateway and Persistence */
...
/**
 * @brief Performs the Lucas-Lehmer test using NTT-accelerated multiplication.
 * @param p The exponent to test (M_p = 2^p - 1).
 * @param out_residue Pointer to store the final residue for verification.
 * @return 1 if prime, 0 if composite, -1 if aborted via signal.
 */
static int ttak_ll_test_core(uint32_t p, uint64_t *out_residue) {
    ...
    /* Allocate memory for the Lucas-Lehmer sequence S_i */
    ...
    for (uint32_t i = 0; i < p - 2; i++) {
    	...
    	/* Combine NTT residues using Chinese Remainder Theorem (CRT) */
    	...
    	/* Handle the (-2) and modular reduction step */
...
/**
 * @brief Worker thread logic for pulling exponents and testing.
 */
void* worker_thread(void* arg) {
    worker_ctx_t *ctx = (worker_ctx_t*)arg;
    while (!atomic_load(&shutdown_requested)) {
    	/* Atomically fetch next exponent and skip even numbers */
    	uint32_t p = atomic_fetch_add(&g_next_p, 2);

    	/* Primality check for the exponent p itself */
    	...
    	if (is_p != -1) {
    		/* Prepare result and telemetry for reporting */
    		...
    		/* Update the global max finished exponent using CAS for thread safety */
    		uint32_t current_max = atomic_load(&g_max_finished_p);
    		while (p > current_max) {
    			if (atomic_compare_exchange_weak(&g_max_finished_p, &current_max, p)) break;
    		}
    	}
...
int main(int argc, char **argv) {
    ...
    printf("[SYSTEM] Initializing Search Engine. Forced Jump to p: %u\n", start_p);
    ...
    while (!atomic_load(&shutdown_requested)) {
    	...
    	uint32_t max_p = atomic_load(&g_max_finished_p);

    	printf("[SYSTEM] Dispatching: %u | Max Finished: %u | Total Ops: %lu\n", cp, max_p, total_ops);
    	fflush(stdout);

    	/* Save Standard Checkpoint (Current Dispatch Point) */
    	char json_cp[256];
    	snprintf(json_cp, sizeof(json_cp), "{\n    \"last_p\": %u,\n    \"total_ops\": %lu\n}\n", cp, total_ops);
    	save_current_progress(CHECKPOINT_FILE, json_cp, strlen(json_cp));

    	/* Save Last Verified Progress (Highest Completed p) */
    	char json_last[256];
    	snprintf(json_last, sizeof(json_last), "{\n    \"max_finished_p\": %u,\n    \"total_ops\": %lu\n}\n", max_p, total_ops);
    	save_current_progress(LAST_FINISHED_FILE, json_last, strlen(json_last));
    }
