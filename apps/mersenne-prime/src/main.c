...
int main(int argc, char **argv) {
    ...
    if (f) {
    	...
    	while (fgets(ln, sizeof(ln), f)) {
    		...
    		if (ptr && (ptr = strchr(ptr, ':'))) {
    			uint32_t val = (uint32_t)atoi(ptr + 1);
    			// Forced Trigger: If the value in JSON is explicitly high (e.g., 1,000,000),
    			// we trust it as a manual jump command.
    			resume_p = val;
    		}
    	}
    	fclose(f);
    ...
    // Ensure we start at an odd number immediately following the resume point
